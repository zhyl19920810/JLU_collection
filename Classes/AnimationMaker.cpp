...
float AnimationMaker::playPlaneAnimation(float delay)
{
    ...
    panel->addChild(plane2);
    panel->addChild(plane3);
...
float AnimationMaker::playAirBattleAnimation(float delay)
{
    int planeNumber = 3;
    int x = 30;
...
float AnimationMaker::playExplodeAnimation(float delay, Point position, int times)
{
    Point p = position;
    for (int i = 0; i < times; i++)
    {
        p = ccp(position.x + i * 15, p.y);
        playExplodeAnimation(delay + i*0.15, p);
        p = ccp(position.x - i * 15, p.y);
        playExplodeAnimation(delay + i*0.15, p);
    }
    return delay+(0.15 + 0.45+0.1)*times;
}
float AnimationMaker::playExplodeAnimation(float delay, Point position)
{
...
void  AnimationMaker::playDamageNumberAnimation(Point position,float delay, bool miss, bool critical, int damage, CallFunc* callback)
{
    Panel*  panel=VIEW_MGR->getPanel(PanelType::BATTLE_MAIN);

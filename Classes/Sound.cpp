...
void Sound::playMusic(const std::string &name, bool bLoop, float volume)
{
    ...
    if (!BUNDLE_MGR->getAppConf()->bgMusicSwitch || m_crtBg == filePath)
        return;
...
{
    if (name.empty() || !BUNDLE_MGR->getAppConf()->effMusicSwitch)
        return;
    string filePath(strRelDir + name);
...
{
    if (!BUNDLE_MGR->getAppConf()->bgMusicSwitch)
        return;
...
{
    if (!BUNDLE_MGR->getAppConf()->effMusicSwitch)
        return;
